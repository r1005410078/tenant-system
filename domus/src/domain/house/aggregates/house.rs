...
impl HouseAggregate {
    ...
    // 创建房源
    pub fn create(new_house: &HouseCreateData) -> (HouseAggregate, HouseCreatedEvent) {
        let house_id = uuid::Uuid::new_v4().to_string();

        (
            HouseAggregate::new(house_id.clone(), new_house.get_address()),
            new_house.to_event(house_id),
        )
    }

    // 删除房源
    pub fn delete(&mut self) -> HouseDeletedEvent {
        self.deleted_at = Some(chrono::Utc::now());
        HouseDeletedEvent::new(self.house_id.clone())
    }

    // 更新房源
    pub fn update(&mut self, new_house: &HouseUpdateData) -> anyhow::Result<HouseUpdatedEvent> {
        if self.is_deleted() {
        ...
        Ok(new_house.to_event())
    }

    // 上架
    pub fn publish(&mut self) -> anyhow::Result<HousePublishedEvent> {
        if self.is_deleted() {
            return Err(anyhow::anyhow!("house is deleted"));
        }
        let publish_at = chrono::Utc::now();
        self.publish_at = Some(publish_at);
        Ok(HousePublishedEvent::new(self.house_id.clone(), publish_at))
    }

    // 下架
    pub fn unpublish(&mut self, description: &str) -> anyhow::Result<HouseUnpublishedEvent> {
        if self.is_deleted() {
            return Err(anyhow::anyhow!("house is deleted"));
        }
